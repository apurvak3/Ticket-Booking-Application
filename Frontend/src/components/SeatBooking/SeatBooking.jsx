...
const SeatBooking = ({ movieTitle, onBookSeats }) => {
  ...
  const seats = Array(8).fill().map(() => Array(8).fill(false));
        ...
        <div className="flex space-x-2">
          {dates.map(date => (
            <button
              ...
              className={`w-10 h-10 rounded-full ${selectedDate === date ? 'bg-red-600' : 'bg-gray-700'}`}
            >
              {date}
            </button>
          ))}
        ...
        <div className="flex flex-wrap gap-2">
          {times.map(time => (
            <button
              ...
              className={`px-4 py-2 rounded ${selectedTime === time ? 'bg-red-600' : 'bg-gray-700'}`}
            >
              {time}
            </button>
          ))}
...
export default SeatBooking;import React, { useState } from 'react';
import { Calendar, Clock } from 'lucide-react';
